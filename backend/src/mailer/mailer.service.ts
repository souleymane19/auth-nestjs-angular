  ...
  async sendCreatedAccountEmail({
    ...
    try {
      const data = await this.mailer.emails.send({
        from: 'samabussiness <onboarding@resend.dev>',
        to: [recipient],
        subject: 'Bienvenue sur la plateforme',
        html: `Bonjour ${firstName}, et bienvenue sur samabussiness! Nous sommes <strong>heureux</strong> de vous avoir parmi nous.`,
      });
